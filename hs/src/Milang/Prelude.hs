...
preludeSrc = T.unlines
  ...
  , "toString val = val -> True = \"True\"; False = \"False\"; Nil = \"[]\"; Nothing = \"Nothing\"; Just = \"Just(\" + toString val.val + \")\"; _ = _toString val"
  , "eq a b = a -> _ = a == b"
  , "id :: a : a"
  , "id x = x"
  , "const :: a : b : a"
  ...
  , "head :: List : Maybe"
  , "head lst = lst -> Nil = Nothing; Cons = Just lst.head"
  , "tail :: List : Maybe"
  , "tail lst = lst -> Nil = Nothing; Cons = Just lst.tail"
  , "fold :: (a : b : a) : a : List : a"
  ...
  , "get :: List : Num : Maybe"
  , "get lst i = lst -> Nil = Nothing; Cons = if (i == 0) ~(Just lst.head) ~(get lst.tail (i - 1))"
  , "sum :: List : Num"
  ...
  , "last :: List : Maybe"
  , "last lst = lst -> Nil = Nothing; Cons = if (null lst.tail) ~(Just lst.head) ~(last lst.tail)"
  , "init :: List : Maybe"
  , "init lst = lst -> Nil = Nothing; Cons = if (null lst.tail) ~(Just []) ~(Just (Cons lst.head ((init lst.tail).val)))"
  , "reverse :: List : List"
