...
exprToC st (ListLit es) = exprToC st (listLitToCons es)
  where
    ...
    listLitToCons (x:xs) = Record "Cons" [Binding "head" False [] x Nothing Nothing Nothing Nothing Nothing,
                                           Binding "tail" False [] (listLitToCons xs) Nothing Nothing Nothing Nothing Nothing]
...
emitPreamble h = hPutStr h $ unlines
  ...
  , ""
  , "// ── Arena allocator ──"
  , "#define MI_ARENA_BLOCK_SIZE (1024 * 1024)  // 1MB blocks"
  ...
  , "            if (!mi_truthy(gv)) continue;  // guard failed"
  , "          }"
  , "          // TCO: tail position"
  ...
  , ""
  , "// ── Built-ins ──"
  , "// truthy: convert any value to 0/1"
  , "// Falsy: 0, 0.0, \"\", False, Nil/[].  Everything else truthy."
  , "static int64_t mi_truthy(MiVal v) {"
  , "  switch (v.type) {"
  ...
  , "}"
  , ""
  , "// slice str/list start end → substring/sublist [start, end)"
  ...
  , "  }"
  , "  /* cons list: drop s, then take (end-s) */"
  , "  MiVal cur = e->val;"
  ...
  , "  if (dlen == 0) {"
  , "    // Split into individual characters — build cons list in reverse then reverse"
  , "    int n = e->str.as.str.len;"
  , "    MiVal lst = mi_nil();"
  , "    for (int i = n - 1; i >= 0; i--) lst = mi_cons(mi_stringn(e->str.as.str.data + i, 1), lst);"
  , "    return lst;"
  , "  }"
  , "  // Collect parts into temp array, then build cons list"
  ...
  , "    case MI_POINTER: snprintf(tmp, sizeof(tmp), \"<ptr:%p>\", v.as.ptr); mi_buf_append(buf,len,cap,tmp); break;"
  , "  }"
  , "}"
  ...
  , "  if (v.type != MI_RECORD) { fprintf(stderr, \"fields: not a record\\n\"); exit(1); }"
  , "  MiVal lst = mi_nil();"
  ...
  , "  (void)env; v = mi_force(v, NULL);"
  , "  if (v.type != MI_RECORD) { fprintf(stderr, \"fieldNames: not a record\\n\"); exit(1); }"
  , "  MiVal lst = mi_nil();"
  ...
  , "  (void)env; v = mi_force(v, NULL);"
  , "  if (v.type != MI_RECORD) { fprintf(stderr, \"tag: not a record\\n\"); exit(1); }"
  ...
  , "  if (name.type != MI_STRING) { fprintf(stderr, \"getField: name must be a string\\n\"); exit(1); }"
  , "  for (int i = 0; i < e->rec.as.rec.nfields; i++) {"
  , "    if (strcmp(e->rec.as.rec.names[i], name.as.str.data) == 0) return e->rec.as.rec.fields[i];"
  , "  }"
  , "  fprintf(stderr, \"getField: field '%s' not found\\n\", name.as.str); exit(1);"
  , "}"
  , "static MiVal mi_builtin_getField(MiVal rec, void *env) {"
  , "  (void)env; rec = mi_force(rec, NULL);"
  , "  if (rec.type != MI_RECORD) { fprintf(stderr, \"getField: not a record\\n\"); exit(1); }"
  ...
  , "  if (name.type != MI_STRING) { fprintf(stderr, \"setField: name must be a string\\n\"); exit(1); }"
  ...
  , "  (void)env; rec = mi_force(rec, NULL);"
  , "  if (rec.type != MI_RECORD) { fprintf(stderr, \"setField: not a record\\n\"); exit(1); }"
