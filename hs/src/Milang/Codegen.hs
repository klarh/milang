...
emitPreamble h = hPutStr h $ unlines
  ...
  , "  MiExpr *e = malloc(sizeof(MiExpr)); e->type = EXPR_RECORD_UPDATE;"
  ...
  , "      if (base.type != MI_RECORD) { fprintf(stderr, \"record update on non-record\\n\"); exit(1); }"
  ...
  , "  if (!f) {"
  , "    char buf[512]; snprintf(buf, sizeof(buf), \"Cannot open file: %s\", path.as.s);"
  , "    return mi_err(buf);"
  , "  }"
  , "  fseek(f, 0, SEEK_END); long len = ftell(f); fseek(f, 0, SEEK_SET);"
  , "  char *contents = malloc(len + 1);"
  , "  long nread = fread(contents, 1, len, f); contents[nread] = '\\0'; fclose(f);"
  , "  return mi_ok(mi_string(contents));"
  , "}"
  , ""
  ...
  , "  FILE *f = fopen(e->path.as.s, \"w\");"
  , "  if (!f) {"
  , "    char buf[512]; snprintf(buf, sizeof(buf), \"Cannot write file: %s\", e->path.as.s);"
  , "    return mi_err(buf);"
  , "  }"
  , "  fputs(contents.as.s, f); fclose(f);"
  ...
  , "  struct mi_wf_env *e = (struct mi_wf_env *)env;"
  , "  FILE *f = fopen(e->path.as.s, \"a\");"
  , "  if (!f) {"
  , "    char buf[512]; snprintf(buf, sizeof(buf), \"Cannot append file: %s\", e->path.as.s);"
  , "    return mi_err(buf);"
  , "  }"
  , "  fputs(contents.as.s, f); fclose(f);"
  , "  return mi_ok(mi_int(0));"
  ...
  , "  (void)env; FILE *f = fopen(path.as.s, \"r\");"
  ...
  , "  (void)env;"
  , "  if (remove(path.as.s) == 0) return mi_ok(mi_int(0));"
  , "  char buf[512]; snprintf(buf, sizeof(buf), \"Cannot remove: %s\", path.as.s);"
  , "  return mi_err(buf);"
  , "}"
  , ""
  ...
  , "  (void)env;"
  , "  if (prompt.type == MI_STRING && strlen(prompt.as.s) > 0) {"
  , "    printf(\"%s\", prompt.as.s); fflush(stdout);"
  , "  }"
  , "  char buf[4096];"
  ...
  , "  (void)env;"
  , "  const char *val = getenv(name.as.s);"
  ...
  , "  char cmdline[8192];"
  , "  int pos = snprintf(cmdline, sizeof(cmdline), \"%s\", e->cmd.as.s);"
  , "  for (int i = 0; i < args.as.list.len && pos < (int)sizeof(cmdline)-1; i++) {"
  , "    MiVal a = args.as.list.items[i];"
  , "    pos += snprintf(cmdline + pos, sizeof(cmdline) - pos, \" %s\", a.as.s);"
  , "  }"
