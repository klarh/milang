...
emitPreamble h = hPutStr h $ unlines
  ...
  , "  EXPR_CASE, EXPR_THUNK, EXPR_VAL, EXPR_RECORD_UPDATE"
  , "} ExprType;"
  , ""
  , "struct MiExpr {"
  , "  ExprType type;"
  ...
  , "  MiExpr *e = mi_alloc(sizeof(MiExpr)); e->type = EXPR_RECORD_UPDATE;"
  ...
  , "      }"
  , "      int total = n + extra;"
  , "      const char **names = mi_alloc(total * sizeof(const char*));"
  , "      MiVal *fields = mi_alloc(total * sizeof(MiVal));"
