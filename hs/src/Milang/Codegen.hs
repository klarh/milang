...
emitPreamble h = hPutStr h $ unlines
  ...
  , "  MiExpr *e = mi_alloc(sizeof(MiExpr)); e->type = EXPR_LIST;"
  , "  e->as.list.len = n; e->as.list.items = mi_alloc(n * sizeof(MiExpr*));"
  , "  va_list args; va_start(args, n);"
  ...
  , "  MiPat *p = mi_alloc(sizeof(MiPat)); p->type = PAT_LIST;"
  , "  p->as.list.npats = n; p->as.list.rest = rest ? mi_strdup(rest) : NULL;"
  , "  p->as.list.pats = mi_alloc(n * sizeof(MiPat*));"
  ...
  , "        MiVal *ritems = mi_alloc(rlen * sizeof(MiVal));"
  ...
  , "      MiVal *items = mi_alloc(n * sizeof(MiVal));"
  ...
  , "  (void)env; struct mi_get_env *e = mi_alloc(sizeof(struct mi_get_env));"
  ...
  , "  int n = e->lst.as.list.len; MiVal *items = mi_alloc((n+1) * sizeof(MiVal));"
  ...
  , "  (void)env; struct mi_push_env *e = mi_alloc(sizeof(struct mi_push_env));"
  ...
  , "  MiVal *items = mi_alloc((na+nb) * sizeof(MiVal));"
  ...
  , "  (void)env; struct mi_concat_env *e = mi_alloc(sizeof(struct mi_concat_env));"
  ...
  , "  int n = lst.as.list.len; MiVal *items = mi_alloc(n * sizeof(MiVal));"
  , "  for (int i = 0; i < n; i++) items[i] = mi_apply(e->fn, lst.as.list.items[i]);"
  , "  return mi_list(items, n);"
  , "}"
  , "static MiVal mi_builtin_map(MiVal fn, void *env) {"
  , "  (void)env; struct mi_map_env *e = mi_alloc(sizeof(struct mi_map_env));"
  ...
  , "  struct mi_fold_env2 *e2 = mi_alloc(sizeof(struct mi_fold_env2));"
  , "  e2->fn = e1->fn; e2->acc = acc; return mi_native_env(mi_builtin_fold3, e2);"
  , "}"
  , "static MiVal mi_builtin_fold(MiVal fn, void *env) {"
  , "  (void)env; struct mi_fold_env1 *e = mi_alloc(sizeof(struct mi_fold_env1));"
  ...
  , "  int n = lst.as.list.len; MiVal *items = mi_alloc(n * sizeof(MiVal)); int count = 0;"
  , "  for (int i = 0; i < n; i++) {"
  ...
  , "  (void)env; struct mi_filter_env *e = mi_alloc(sizeof(struct mi_filter_env));"
  ...
  , "    p = found + dlen;"
  , "  }"
  , "  MiVal *aitems = (MiVal*)mi_alloc(count * sizeof(MiVal));"
  , "  memcpy(aitems, items, count * sizeof(MiVal));"
  , "  free(items);"
  , "  return mi_list(aitems, count);"
  , "}"
  ...
  , "  }"
  , "  char *buf = mi_alloc(total + 1); int pos = 0;"
  , "  for (int i = 0; i < n; i++) {"
  ...
  , "  (void)env; struct mi_join_env *e = mi_alloc(sizeof(struct mi_join_env));"
  ...
  , "  (void)env; struct mi_min_env *e = mi_alloc(sizeof(struct mi_min_env));"
  ...
  , "  (void)env; struct mi_max_env *e = mi_alloc(sizeof(struct mi_max_env));"
  ...
  , "  MiVal lst; lst.type = MI_LIST;"
  , "  lst.as.list.len = v.as.rec.nfields;"
  , "  lst.as.list.items = mi_alloc(sizeof(MiVal) * v.as.rec.nfields);"
  , "  for (int i = 0; i < v.as.rec.nfields; i++) lst.as.list.items[i] = v.as.rec.fields[i];"
  , "  return lst;"
  , "}"
  ...
  , "  lst.as.list.items = mi_alloc(sizeof(MiVal) * v.as.rec.nfields);"
  , "  for (int i = 0; i < v.as.rec.nfields; i++) {"
  , "    lst.as.list.items[i] = mi_string(v.as.rec.names[i]);"
  , "  }"
  , "  return lst;"
  , "}"
  ...
  , "  // Build argv list"
  , "  MiVal *argItems = mi_alloc(argc * sizeof(MiVal));"
