...
emitPreamble h = hPutStr h $ unlines
  ...
  , "// ── String built-ins ──"
  , ""
  , "// charAt str idx → Just single-character string, or Nothing if out of range"
  ...
  , "}"
  , ""
  , "// toInt val → Just integer, or Nothing if conversion fails"
  ...
  , "}"
  , ""
  , "// toFloat val → Just float, or Nothing if conversion fails"
  ...
  , "}"
  , ""
  , "// world.fs.read.file path → Ok {value=contents} / Err {msg=...}"
  ...
  , "}"
  , ""
  , "// world.fs.write.file path contents → Ok {value=0} / Err {msg=...}"
  ...
  , "}"
  , ""
  , "// world.fs.write.append path contents → Ok {value=0} / Err {msg=...}"
  ...
  , "}"
  , ""
  , "// world.fs.write.remove path → Ok {value=0} / Err {msg=...}"
  ...
  , ""
  , "  // Build world.io record (console I/O)"
  , "  const int io_n = 3;"
