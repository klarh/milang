...
emitPreamble h = hPutStr h $ unlines
  ...
  , "    case MI_SIZED_INT: if (v.as.sized.is_big) { int bsz = v.as.sized.big->len*10+16; char *t = (char*)mi_alloc(bsz); mi_bn_tostr(v.as.sized.big, t, bsz); printf(\"%s\", t); } else printf(\"%ld\", v.as.sized.i); break;"
  ...
  , "    case MI_SIZED_INT: if (v.as.sized.is_big) { int bsz = v.as.sized.big->len*10+16; char *t2 = (char*)mi_alloc(bsz); mi_bn_tostr(v.as.sized.big, t2, bsz); mi_buf_append(buf,len,cap,t2); } else { snprintf(tmp, sizeof(tmp), \"%ld\", v.as.sized.i); mi_buf_append(buf,len,cap,tmp); } break;"
