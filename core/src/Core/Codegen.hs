...
emitPreamble h = hPutStr h $ unlines
  ...
  , "  void *raw = calloc(1, MI_NENV_HDR + size);"
  , "  void *env = (char*)raw + MI_NENV_HDR;"
  , "  MiNenvNode *node = (MiNenvNode*)malloc(sizeof(MiNenvNode));"
  ...
  , "      if (n == 0) { *dst = r; break; }"
  , "      r.as.rec.names = (const char**)malloc(n * sizeof(const char*));"
  , "      r.as.rec.fields = (MiVal*)malloc(n * sizeof(MiVal));"
  ...
  , "      void *raw = calloc(1, MI_NENV_HDR + n * sizeof(MiVal));"
  ...
  , "    mi_gc_val_globals = realloc(mi_gc_val_globals, mi_gc_val_global_cap * sizeof(MiVal*));"
  , "  }"
  ...
  , "    char *s = (char*)malloc(v.as.str.len + 1);"
