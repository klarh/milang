...
emitPreamble h = hPutStr h $ unlines
  ...
  , "typedef enum { MI_INT, MI_FLOAT, MI_FLOAT32, MI_STRING, MI_RECORD, MI_CLOSURE, MI_NATIVE, MI_POINTER, MI_MANAGED } MiType;"
  , ""
  , "struct MiVal {"
  , "  MiType type;"
  , "  union {"
  ...
  , "    if (strcmp(op, \"*\") == 0) return mi_float32(fa * fb);"
  , "    if (strcmp(op, \"/\") == 0) return mi_float32(fa / fb);"
  , "    if (strcmp(op, \"**\") == 0 && b.type == MI_INT && b.as.i >= 0) { float r=1.0f; for(int64_t e=b.as.i;e>0;e--) r*=fa; return mi_float32(r); }"
  , "    if (strcmp(op, \"<\") == 0) return mi_int(fa < fb);"
  , "    if (strcmp(op, \">\") == 0) return mi_int(fa > fb);"
  , "    if (strcmp(op, \"<=\") == 0) return mi_int(fa <= fb);"
  , "    if (strcmp(op, \">=\") == 0) return mi_int(fa >= fb);"
