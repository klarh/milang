...
emitPreamble h = hPutStr h $ unlines
  ...
  , "#include <math.h>"
  , ""
  , "// ── Safe allocation wrappers (abort on OOM) ──"
  , "static void *mi_safe_malloc(size_t size) {"
  , "  void *p = malloc(size);"
  , "  if (!p) { fprintf(stderr, \"milang: out of memory (malloc %zu bytes)\\n\", size); abort(); }"
  , "  return p;"
  , "}"
  , "static void *mi_safe_calloc(size_t n, size_t size) {"
  , "  void *p = calloc(n, size);"
  , "  if (!p) { fprintf(stderr, \"milang: out of memory (calloc %zu*%zu bytes)\\n\", n, size); abort(); }"
  , "  return p;"
  , "}"
  , "static void *mi_safe_realloc(void *ptr, size_t size) {"
  , "  void *p = realloc(ptr, size);"
  , "  if (!p) { fprintf(stderr, \"milang: out of memory (realloc %zu bytes)\\n\", size); abort(); }"
  , "  return p;"
  , "}"
  , ""
  , "// ── Arena allocator ──"
  , "#define MI_ARENA_BLOCK_SIZE (1024 * 1024)"
  , "#define MI_ARENA_GC_THRESHOLD (64 * 1024 * 1024)"
  ...
  , "static size_t mi_arena_total = 0;"
  , "static size_t mi_arena_gc_next = MI_ARENA_GC_THRESHOLD;"
  , "static int mi_arena_gc_running = 0;"
  , ""
  ...
  , "    b = (MiArenaBlock*)mi_safe_malloc(sizeof(MiArenaBlock) + cap);"
  ...
  , "}"
  , ""
  , "// Permanent allocator for program structures (MiExpr, MiPat, MiBinding)"
  , "// These are created at startup and must survive arena GC."
  , "static void *mi_perm_alloc(size_t size) {"
  , "  void *p = mi_safe_calloc(1, size); return p;"
  , "}"
  , "static char *mi_perm_strdup(const char *s) {"
  , "  size_t n = strlen(s) + 1;"
  , "  char *p = (char*)mi_safe_malloc(n);"
  , "  memcpy(p, s, n);"
  , "  return p;"
  , "}"
  , ""
  ...
  , "    struct { MiVal (*fn)(MiVal, void*); void *env; int env_nfields; } native;"
  , "    void *ptr;"
  ...
  , "  MiGcManaged *m = (MiGcManaged*)mi_safe_malloc(sizeof(MiGcManaged));"
  ...
  , "  MiVal r; r.type = MI_NATIVE; r.as.native.fn = fn; r.as.native.env = NULL; r.as.native.env_nfields = 0; return r;"
  , "}"
  , "static MiVal mi_native_env(MiVal (*fn)(MiVal, void*), void *env) {"
  , "  MiVal r; r.type = MI_NATIVE; r.as.native.fn = fn; r.as.native.env = env; r.as.native.env_nfields = 0; return r;"
  , "}"
  , ""
  , "// ── Heap-allocated native closure environments ──"
  , "// Native env layout: [gc_epoch (8 bytes, aligned)] [MiVal fields...]"
  , "#define MI_NENV_HDR 8"
  , "typedef struct MiNenvNode { void *raw; int nfields; struct MiNenvNode *next; } MiNenvNode;"
  , "static MiNenvNode *mi_nenv_list = NULL;"
  , "static MiNenvNode *mi_nenv_deferred = NULL;"
  , "static int mi_gc_epoch = 0;"
  , "// Nenv mark stack: scoped sweep boundaries"
  , "#define MI_NENV_MARK_STACK_SIZE 256"
  , "static MiNenvNode *mi_nenv_marks[MI_NENV_MARK_STACK_SIZE];"
  , "static int mi_nenv_mark_count = 0;"
  , "static void mi_nenv_push_mark(void) {"
  , "  if (mi_nenv_mark_count < MI_NENV_MARK_STACK_SIZE)"
  , "    mi_nenv_marks[mi_nenv_mark_count++] = mi_nenv_list;"
  , "}"
  , "static void mi_nenv_pop_mark(void) {"
  , "  if (mi_nenv_mark_count > 0) {"
  , "    MiNenvNode *mark = mi_nenv_marks[--mi_nenv_mark_count];"
  , "    // Move nodes from this scope to deferred list for lazy cleanup"
  , "    while (mi_nenv_list && mi_nenv_list != mark) {"
  , "      MiNenvNode *nn = mi_nenv_list;"
  , "      mi_nenv_list = nn->next;"
  , "      nn->next = mi_nenv_deferred;"
  , "      mi_nenv_deferred = nn;"
  , "    }"
  , "  }"
  , "}"
  , "static void *mi_nenv_alloc(size_t size) {"
  , "  void *raw = mi_safe_calloc(1, MI_NENV_HDR + size);"
  , "  void *env = (char*)raw + MI_NENV_HDR;"
  , "  MiNenvNode *node = (MiNenvNode*)mi_safe_malloc(sizeof(MiNenvNode));"
  , "  node->raw = raw; node->nfields = (int)(size / sizeof(MiVal));"
  , "  node->next = mi_nenv_list; mi_nenv_list = node;"
  , "  return env;"
  , "}"
  , "static MiVal mi_native_env_n(MiVal (*fn)(MiVal, void*), void *env, int nfields) {"
  , "  MiVal r; r.type = MI_NATIVE; r.as.native.fn = fn; r.as.native.env = env; r.as.native.env_nfields = nfields; return r;"
  , "}"
  , ""
  , "// ── Error helpers ──"
  ...
  , "  case MI_STRING: {"
  , "    MiVal r = v;"
  , "    r.as.str.data = (char*)mi_safe_malloc(v.as.str.len + 1);"
  ...
  , "    for (;;) {"
  , "      MiVal r = cur;"
  , "      int n = cur.as.rec.nfields;"
  , "      if (n == 0) { *dst = r; break; }"
  , "      r.as.rec.names = (const char**)mi_safe_malloc(n * sizeof(const char*));"
  , "      r.as.rec.fields = (MiVal*)mi_safe_malloc(n * sizeof(MiVal));"
  , "      memcpy((void*)r.as.rec.names, cur.as.rec.names, n * sizeof(const char*));"
  , "      for (int i = 0; i < n - 1; i++)"
  , "        r.as.rec.fields[i] = mi_val_deep_copy(cur.as.rec.fields[i]);"
  , "      *dst = r;"
  , "      MiVal tail = cur.as.rec.fields[n - 1];"
  , "      if (tail.type == MI_RECORD) {"
  , "        dst = &r.as.rec.fields[n - 1];"
  , "        cur = tail;"
  , "      } else {"
  , "        r.as.rec.fields[n - 1] = mi_val_deep_copy(tail);"
  , "        break;"
  , "      }"
  , "    }"
  , "    return root;"
  ...
  , "    MiBignum *nb = (MiBignum*)mi_safe_malloc(sizeof(MiBignum));"
  , "    nb->digits = (uint32_t*)mi_safe_malloc(b->cap * sizeof(uint32_t));"
  , "    memcpy(nb->digits, b->digits, b->len * sizeof(uint32_t));"
  , "    nb->len = b->len; nb->cap = b->cap; nb->sign = b->sign;"
  , "    r.as.sized.big = nb; return r;"
  , "  }"
  , "  case MI_NATIVE: {"
  , "    if (v.as.native.env && v.as.native.env_nfields > 0) {"
  , "      int n = v.as.native.env_nfields;"
  , "      void *raw = mi_safe_calloc(1, MI_NENV_HDR + n * sizeof(MiVal));"
  , "      MiVal *nf = (MiVal*)((char*)raw + MI_NENV_HDR);"
  , "      MiVal *of = (MiVal*)v.as.native.env;"
  , "      for (int i = 0; i < n; i++) nf[i] = mi_val_deep_copy(of[i]);"
  , "      MiVal r = v; r.as.native.env = nf; return r;"
  , "    }"
  , "    return v;"
  , "  }"
  , "  default: return v;"
  ...
  , "  for (;;) {"
  , "    switch (cur.type) {"
  , "    case MI_STRING: free(cur.as.str.data); return;"
  , "    case MI_RECORD: {"
  , "      int n = cur.as.rec.nfields;"
  , "      for (int i = 0; i < n - 1; i++) mi_val_free_deep(cur.as.rec.fields[i]);"
  , "      if (n > 0) {"
  , "        MiVal tail = cur.as.rec.fields[n - 1];"
  , "        free((void*)cur.as.rec.names); free(cur.as.rec.fields);"
  , "        if (tail.type == MI_RECORD) { cur = tail; continue; }"
  , "        mi_val_free_deep(tail);"
  , "      } else {"
  , "        free((void*)cur.as.rec.names); free(cur.as.rec.fields);"
  , "      }"
  , "      return;"
  , "    }"
  , "    case MI_SIZED_INT:"
  , "      if (cur.as.sized.is_big) { free(cur.as.sized.big->digits); free(cur.as.sized.big); }"
  , "      return;"
  , "    case MI_NATIVE:"
  , "      if (cur.as.native.env && cur.as.native.env_nfields > 0) {"
  , "        MiVal *f = (MiVal*)cur.as.native.env;"
  , "        for (int i = 0; i < cur.as.native.env_nfields; i++) mi_val_free_deep(f[i]);"
  , "        free((char*)cur.as.native.env - MI_NENV_HDR);"
  , "      }"
  , "      return;"
  , "    default: return;"
  , "    }"
  ...
  , "      memcpy((void*)r.as.rec.names, cur.as.rec.names, n * sizeof(const char*));"
  , "      for (int i = 0; i < n - 1; i++)"
  , "        r.as.rec.fields[i] = mi_val_to_arena(cur.as.rec.fields[i]);"
  , "      *dst = r;"
  , "      MiVal tail = cur.as.rec.fields[n - 1];"
  , "      if (tail.type == MI_RECORD) {"
  , "        dst = &r.as.rec.fields[n - 1];"
  , "        cur = tail;"
  , "      } else {"
  , "        r.as.rec.fields[n - 1] = mi_val_to_arena(tail);"
  , "        break;"
  , "      }"
  , "    }"
  , "    return root;"
  ...
  , "  case MI_NATIVE: {"
  , "    if (v.as.native.env && v.as.native.env_nfields > 0) {"
  , "      int *epoch = (int*)((char*)v.as.native.env - MI_NENV_HDR);"
  , "      if (*epoch != mi_gc_epoch) {"
  , "        *epoch = mi_gc_epoch;"
  , "        MiVal *f = (MiVal*)v.as.native.env;"
  , "        for (int i = 0; i < v.as.native.env_nfields; i++)"
  , "          f[i] = mi_val_to_arena(f[i]);"
  , "      }"
  , "    }"
  , "    return v;"
  , "  }"
  ...
  , ""
  , "// Shadow stack: GC roots for MiVal locals on the C stack (dynamic)"
  , "#define MI_ROOT_INIT_CAP 4096"
  , "static MiVal **mi_gc_roots = NULL;"
  , "static int mi_gc_root_count = 0;"
  , "static int mi_gc_root_cap = 0;"
  , "static void mi_gc_root_push(MiVal *v) {"
  , "  if (mi_gc_root_count >= mi_gc_root_cap) {"
  , "    mi_gc_root_cap = mi_gc_root_cap ? mi_gc_root_cap * 2 : MI_ROOT_INIT_CAP;"
  , "    mi_gc_roots = (MiVal**)mi_safe_realloc(mi_gc_roots, mi_gc_root_cap * sizeof(MiVal*));"
  , "  }"
  , "  mi_gc_roots[mi_gc_root_count++] = v;"
  , "}"
  , ""
  , "// Shadow stack: GC roots for MiEnv* locals on the C stack (dynamic)"
  , "static MiEnv **mi_gc_env_roots = NULL;"
  , "static int mi_gc_env_root_count = 0;"
  , "static int mi_gc_env_root_cap = 0;"
  , "static void mi_gc_env_root_push(MiEnv *e) {"
  , "  if (mi_gc_env_root_count >= mi_gc_env_root_cap) {"
  , "    mi_gc_env_root_cap = mi_gc_env_root_cap ? mi_gc_env_root_cap * 2 : MI_ROOT_INIT_CAP;"
  , "    mi_gc_env_roots = (MiEnv**)mi_safe_realloc(mi_gc_env_roots, mi_gc_env_root_cap * sizeof(MiEnv*));"
  , "  }"
  , "  mi_gc_env_roots[mi_gc_env_root_count++] = e;"
  , "}"
  , ""
  ...
  , "    mi_gc_pinned = mi_safe_realloc(mi_gc_pinned, mi_gc_pin_cap * sizeof(MiVal*));"
  , "  }"
  ...
  , "  else { e = (MiEnv*)mi_safe_malloc(sizeof(MiEnv)); }"
  ...
  , "  } else if (v.type == MI_NATIVE && v.as.native.env && v.as.native.env_nfields > 0) {"
  , "    MiVal *f = (MiVal*)v.as.native.env;"
  , "    for (int i = 0; i < v.as.native.env_nfields - 1; i++)"
  , "      mi_gc_scan_val(f[i]);"
  , "    if (v.as.native.env_nfields > 0) { v = f[v.as.native.env_nfields - 1]; goto restart; }"
  , "  }"
  , "}"
  ...
  , "}"
  , ""
  , "// ── Arena value GC (copying collector) ──"
  , "// Global root table: pointers to static MiVal globals"
  , "static MiVal **mi_gc_val_globals = NULL;"
  , "static int mi_gc_val_global_count = 0;"
  , "static int mi_gc_val_global_cap = 0;"
  , "static void mi_gc_register_global(MiVal *v) {"
  , "  if (mi_gc_val_global_count >= mi_gc_val_global_cap) {"
  , "    mi_gc_val_global_cap = mi_gc_val_global_cap ? mi_gc_val_global_cap * 2 : 64;"
  , "    mi_gc_val_globals = mi_safe_realloc(mi_gc_val_globals, mi_gc_val_global_cap * sizeof(MiVal*));"
  , "  }"
  , "  mi_gc_val_globals[mi_gc_val_global_count++] = v;"
  , "}"
  , ""
  , "static void mi_gc_copy_env_vals(MiEnv *env) {"
  , "  while (env) {"
  , "    for (MiEnv *e = env; e; e = e->next) {"
  , "      if (e->name) e->val = mi_val_to_arena(e->val);"
  , "    }"
  , "    env = env->parent;"
  , "  }"
  , "}"
  , ""
  , "static void mi_gc_collect_arena(void) {"
  , "  if (mi_arena_gc_running) return;"
  , "  mi_arena_gc_running = 1;"
  , "  mi_gc_epoch++;"
  , "  MiArenaBlock *old = mi_arena.head;"
  , "  mi_arena.head = NULL;"
  , "  mi_arena_total = 0;"
  , "  // Copy global roots"
  , "  for (int i = 0; i < mi_gc_val_global_count; i++)"
  , "    *mi_gc_val_globals[i] = mi_val_to_arena(*mi_gc_val_globals[i]);"
  , "  // Copy env root values"
  , "  for (int i = 0; i < mi_gc_env_root_count; i++)"
  , "    mi_gc_copy_env_vals(mi_gc_env_roots[i]);"
  , "  // Copy stack roots"
  , "  for (int i = 0; i < mi_gc_root_count; i++)"
  , "    *mi_gc_roots[i] = mi_val_to_arena(*mi_gc_roots[i]);"
  , "  // Copy pinned roots"
  , "  for (int i = 0; i < mi_gc_pin_count; i++)"
  , "    *mi_gc_pinned[i] = mi_val_to_arena(*mi_gc_pinned[i]);"
  , "  // Walk ALL live envs (from mi_gc_all list)"
  , "  for (MiEnv *e = mi_gc_all; e; e = e->gc_all) {"
  , "    if (e->name) e->val = mi_val_to_arena(e->val);"
  , "  }"
  , "  // Scoped nenv sweep: free unreachable nodes within innermost mark boundary."
  , "  // Nodes before boundary: reachable ones already updated by root scanning;"
  , "  // dead ones are skipped to avoid copying dead data to new arena."
  , "  MiNenvNode *_nenv_boundary = (mi_nenv_mark_count > 0) ? mi_nenv_marks[mi_nenv_mark_count - 1] : NULL;"
  , "  { MiNenvNode **_np = &mi_nenv_list;"
  , "    while (*_np && *_np != _nenv_boundary) {"
  , "      MiNenvNode *nn = *_np;"
  , "      int *ep = (int*)nn->raw;"
  , "      if (*ep == mi_gc_epoch) { _np = &nn->next; }"
  , "      else { *_np = nn->next; free(nn->raw); free(nn); }"
  , "    }"
  , "  }"
  , "  // Sweep deferred nenv list: free dead nodes from completed scopes"
  , "  { MiNenvNode **_dp = &mi_nenv_deferred;"
  , "    while (*_dp) {"
  , "      MiNenvNode *nn = *_dp;"
  , "      int *ep = (int*)nn->raw;"
  , "      if (*ep == mi_gc_epoch) { _dp = &nn->next; }"
  , "      else { *_dp = nn->next; free(nn->raw); free(nn); }"
  , "    }"
  , "  }"
  , "  // Free old arena blocks"
  , "  while (old) { MiArenaBlock *n = old->next; free(old); old = n; }"
  , "  // Dynamic threshold: next GC at max(base, 2x live data)"
  , "  mi_arena_gc_next = mi_arena_total * 2;"
  , "  if (mi_arena_gc_next < MI_ARENA_GC_THRESHOLD) mi_arena_gc_next = MI_ARENA_GC_THRESHOLD;"
  , "  mi_arena_gc_running = 0;"
  , "}"
  , ""
  , "// ── Expr constructors ──"
  , "static MiExpr *mi_expr_int(int64_t v) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_INT; e->as.i = v; return e;"
  , "}"
  , "static MiExpr *mi_expr_sized_int(int64_t v, int width, int is_signed) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_SIZED_INT;"
  , "  e->as.sized.i = v; e->as.sized.width = width; e->as.sized.is_signed = is_signed; return e;"
  , "}"
  , "static MiExpr *mi_expr_float(double v) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_FLOAT; e->as.f = v; return e;"
  , "}"
  , "static MiExpr *mi_expr_float32(float v) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_FLOAT32; e->as.f = (double)v; return e;"
  , "}"
  , "static MiExpr *mi_expr_string(const char *s) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_STRING; e->as.s = mi_perm_strdup(s); return e;"
  , "}"
  , "static MiExpr *mi_expr_name(const char *n) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_NAME; e->as.name = mi_perm_strdup(n); return e;"
  , "}"
  , "static MiExpr *mi_expr_binop(const char *op, MiExpr *l, MiExpr *r) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_BINOP;"
  , "  e->as.binop.op = mi_perm_strdup(op); e->as.binop.left = l; e->as.binop.right = r; return e;"
  , "}"
  , "static MiExpr *mi_expr_app(MiExpr *fn, MiExpr *arg) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_APP;"
  , "  e->as.app.fn = fn; e->as.app.arg = arg; return e;"
  , "}"
  , "static MiExpr *mi_expr_lam(const char *param, MiExpr *body) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_LAM;"
  , "  e->as.lam.param = mi_perm_strdup(param); e->as.lam.body = body; return e;"
  , "}"
  , "static MiExpr *mi_expr_val(MiVal v) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_VAL;"
  , "  if (v.type == MI_STRING && v.as.str.data) {"
  , "    char *s = (char*)mi_safe_malloc(v.as.str.len + 1);"
  , "    memcpy(s, v.as.str.data, v.as.str.len + 1);"
  , "    v.as.str.data = s;"
  , "  }"
  , "  e->as.val = v; return e;"
  , "}"
  , "static MiExpr *mi_expr_field(MiExpr *expr, const char *field) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_FIELD;"
  , "  e->as.field.expr = expr; e->as.field.field = mi_perm_strdup(field); return e;"
  , "}"
  , "static MiExpr *mi_expr_thunk(MiExpr *body) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_THUNK; e->as.thunk.body = body; return e;"
  , "}"
  , ""
  , "static MiBinding mi_binding(const char *name, int lazy, int nparams, ...) {"
  , "  MiBinding b; b.name = mi_perm_strdup(name); b.lazy = lazy; b.nparams = nparams;"
  , "  va_list args; va_start(args, nparams);"
  , "  if (nparams > 0) {"
  , "    b.params = mi_perm_alloc(nparams * sizeof(char*));"
  , "    for (int i = 0; i < nparams; i++) b.params[i] = mi_perm_strdup(va_arg(args, char*));"
  , "  } else { b.params = NULL; }"
  , "  b.body = va_arg(args, MiExpr*); va_end(args); return b;"
  , "}"
  , ""
  , "static MiExpr *mi_expr_with(MiExpr *body, int n, ...) {"
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_WITH;"
  , "  e->as.with.body = body; e->as.with.nbindings = n;"
  , "  e->as.with.bindings = mi_perm_alloc(n * sizeof(MiBinding));"
  ...
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_RECORD;"
  , "  e->as.record.tag = mi_perm_strdup(tag); e->as.record.nbindings = n;"
  , "  e->as.record.bindings = mi_perm_alloc(n * sizeof(MiBinding));"
  , "  va_list args; va_start(args, n);"
  ...
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_NAMESPACE;"
  , "  e->as.ns.nbindings = n; e->as.ns.bindings = mi_perm_alloc(n * sizeof(MiBinding));"
  , "  va_list args; va_start(args, n);"
  ...
  , "  MiPat *p = mi_perm_alloc(sizeof(MiPat)); p->type = PAT_VAR; p->as.var = mi_perm_strdup(v); return p;"
  , "}"
  , "static MiPat *mi_pat_wild(void) { MiPat *p = mi_perm_alloc(sizeof(MiPat)); p->type = PAT_WILD; return p; }"
  , "static MiPat *mi_pat_int(int64_t v) {"
  , "  MiPat *p = mi_perm_alloc(sizeof(MiPat)); p->type = PAT_INT; p->as.i = v; return p;"
  , "}"
  , "static MiPat *mi_pat_string(const char *s) {"
  , "  MiPat *p = mi_perm_alloc(sizeof(MiPat)); p->type = PAT_STRING; p->as.s = mi_perm_strdup(s); return p;"
  , "}"
  , "static MiPat *mi_pat_rec(const char *tag, int n, ...) {"
  , "  MiPat *p = mi_perm_alloc(sizeof(MiPat)); p->type = PAT_REC;"
  , "  p->as.rec.tag = mi_perm_strdup(tag); p->as.rec.nfields = n;"
  , "  p->as.rec.field_names = mi_perm_alloc(n * sizeof(char*));"
  , "  p->as.rec.field_pats = mi_perm_alloc(n * sizeof(MiPat*));"
  , "  va_list args; va_start(args, n);"
  , "  for (int i = 0; i < n; i++) {"
  , "    p->as.rec.field_names[i] = mi_perm_strdup(va_arg(args, char*));"
  ...
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_CASE;"
  , "  e->as.cas.scrut = scrut; e->as.cas.nalts = n;"
  , "  e->as.cas.alts = mi_perm_alloc(n * sizeof(MiAlt));"
  , "  va_list args; va_start(args, n);"
  ...
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_IF;"
  ...
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_FOR_RANGE;"
  , "  e->as.for_range.acc_var = mi_perm_strdup(acc_var);"
  , "  e->as.for_range.iter_var = mi_perm_strdup(iter_var);"
  ...
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_TAIL_LOOP;"
  ...
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_TAIL_CALL;"
  , "  e->as.tail_call.nargs = nargs;"
  , "  e->as.tail_call.args = mi_perm_alloc(nargs * sizeof(MiExpr*));"
  ...
  , "  MiExpr *e = mi_perm_alloc(sizeof(MiExpr)); e->type = EXPR_NATIVE_BINOP;"
  , "  e->as.native_binop.op = mi_perm_strdup(op);"
  ...
  , "        if (mi_arena_total > mi_arena_gc_next) {"
  , "          mi_env_set(loop_env, expr->as.for_range.acc_var, acc);"
  , "          mi_gc_collect_arena();"
  , "          acc = mi_env_get(loop_env, expr->as.for_range.acc_var);"
  , "        }"
  ...
  , "      MiVal *_targs = (MiVal*)mi_safe_malloc(_tn * sizeof(MiVal));"
  , "      mi_gc_env_root_push(env);"
  , "      for (int _ti = 0; _ti < _tn; _ti++) {"
  ...
  , "  MiVal *items = (MiVal*)mi_safe_malloc(cap * sizeof(MiVal));"
  ...
  , "    if (count >= cap) { cap *= 2; items = (MiVal*)mi_safe_realloc(items, cap * sizeof(MiVal)); }"
  ...
  , "  int slen = strlen(s);"
  , "  while (*len + slen >= *cap) { *cap *= 2; *buf = mi_safe_realloc(*buf, *cap); }"
  ...
  , "    case MI_STRING: {"
  , "      while (*len + v.as.str.len >= *cap) { *cap *= 2; *buf = mi_safe_realloc(*buf, *cap); }"
