    ...
    Namespace bs -> do
      ...
      if hasMainWithArg
        then do
          ...
          emit "  return (_result.type == MI_INT) ? (int)_result.as.i : ((_result.type == MI_SIZED_INT) ? (int)_result.as.sized.i : 0);\n}\n"
        else do
          mapM_ (\b -> do
...
exprToC _ (SizedInt n w s) = pure $ "mi_expr_sized_int(" ++ show n ++ ", " ++ show w ++ ", " ++ (if s then "1" else "0") ++ ")"
...
miValToC (CInt w) name  = "(" ++ cIntTypeName w ++ ")((" ++ name ++ ".type == MI_SIZED_INT) ? " ++ name ++ ".as.sized.i : " ++ name ++ ".as.i)"
miValToC (CUInt w) name = "(" ++ cUIntTypeName w ++ ")((uint64_t)((" ++ name ++ ".type == MI_SIZED_INT) ? " ++ name ++ ".as.sized.i : " ++ name ++ ".as.i))"
...
miToCArg (CInt w) name  = "(" ++ cIntTypeName w ++ ")((" ++ name ++ ".type == MI_SIZED_INT) ? " ++ name ++ ".as.sized.i : " ++ name ++ ".as.i)"
miToCArg (CUInt w) name = "(" ++ cUIntTypeName w ++ ")((uint64_t)((" ++ name ++ ".type == MI_SIZED_INT) ? " ++ name ++ ".as.sized.i : " ++ name ++ ".as.i))"
...
emitPreamble h = hPutStr h $ unlines
  ...
  , "  MiVal r; r.type = MI_SIZED_INT;"
  , "  r.as.sized.i = s ? mi_clamp_signed(w, v) : (int64_t)mi_clamp_unsigned(w, v);"
  ...
  , "  if (v.type == MI_SIZED_INT) return (double)v.as.sized.i;"
  ...
  , "  if (v.type == MI_SIZED_INT) return (float)v.as.sized.i;"
  , "  return (float)v.as.i;"
  , "}"
  , ""
  ...
  , "    case MI_SIZED_INT: printf(\"%ld\", v.as.sized.i); break;"
  ...
  , "    case PAT_INT:    return (val.type == MI_INT && val.as.i == pat->as.i) || (val.type == MI_SIZED_INT && val.as.sized.i == pat->as.i);"
  ...
  , "    if (strcmp(op, \"*\") == 0) return mi_float(fa * fb);"
  , "    if (strcmp(op, \"/\") == 0) return mi_float(fa / fb);"
  , "    if (strcmp(op, \"**\") == 0 && (b.type == MI_INT || b.type == MI_SIZED_INT)) { int64_t bi = (b.type == MI_SIZED_INT) ? b.as.sized.i : b.as.i; if (bi >= 0) { double r=1.0; for(int64_t e=bi;e>0;e--) r*=fa; return mi_float(r); } }"
  ...
  , "    if (strcmp(op, \"*\") == 0) return mi_float32(fa * fb);"
  , "    if (strcmp(op, \"/\") == 0) return mi_float32(fa / fb);"
  , "    if (strcmp(op, \"**\") == 0 && (b.type == MI_INT || b.type == MI_SIZED_INT)) { int64_t bi = (b.type == MI_SIZED_INT) ? b.as.sized.i : b.as.i; if (bi >= 0) { float r=1.0f; for(int64_t e=bi;e>0;e--) r*=fa; return mi_float32(r); } }"
  , "    if (strcmp(op, \"<\") == 0) return mi_int(fa < fb);"
  , "    if (strcmp(op, \">\") == 0) return mi_int(fa > fb);"
  , "    if (strcmp(op, \"<=\") == 0) return mi_int(fa <= fb);"
  , "    if (strcmp(op, \">=\") == 0) return mi_int(fa >= fb);"
  ...
  , "    int w = (a.type == MI_SIZED_INT) ? a.as.sized.width : ((b.type == MI_SIZED_INT) ? b.as.sized.width : 64);"
  ...
  , "    case MI_SIZED_INT: return v.as.sized.i != 0;"
  ...
  , "    case MI_SIZED_INT: snprintf(tmp, sizeof(tmp), \"%ld\", v.as.sized.i); mi_buf_append(buf,len,cap,tmp); break;"
  ...
  , "  if (v.type == MI_SIZED_INT) return mi_just(mi_int(v.as.sized.i));"
  ...
  , "  if (v.type == MI_SIZED_INT) return mi_float((double)v.as.sized.i);"
  ...
  , "  return mi_sized_int(val.as.i, e->width, 1);"
  , "}"
  ...
  , "  struct mi_sized_env *e = (struct mi_sized_env *)env;"
  , "  return mi_sized_int(val.as.i, e->width, 0);"
  , "}"
