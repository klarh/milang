...
preludeSrc = T.unlines
  ...
  , "len x = x -> Nil = 0; Cons h t = 1 + len t; Nothing = 0; Just v = 1; _ = strlen x"
  ...
  , "last lst = lst -> Nil = Nothing; Cons h (Nil) = Just h; Cons h t = last t"
  ...
  , "foldRight f z lst = _foldRight_cps f z lst (\\x -> x)"
  , "_foldRight_cps f z lst k = lst -> Nil = k z; Cons h t = _foldRight_cps f z t (\\acc -> k (f h acc))"
  ...
  , "fst pair = pair.fst"
  , "snd pair = pair.snd"
  , "sortBy cmp lst = lst -> Nil = []; Cons h t = _sortBy_check cmp h t"
  , "_sortBy_check cmp h t = t -> Nil = [h]; _ = _mergeSort cmp (Cons h t)"
  , "_mergeSort cmp lst = _mergeSort_do cmp (_mergeSort_split lst [] [])"
  , "_mergeSort_do cmp parts = _mergeSort_merge cmp (sortBy cmp parts.left) (sortBy cmp parts.right) []"
  , "_mergeSort_split lst a b = lst -> Nil = {left = a; right = b}; Cons h t = _mergeSort_split t b (Cons h a)"
  , "_mergeSort_merge cmp xs ys acc = if (null xs) (concat (reverse acc) ys) (if (null ys) (concat (reverse acc) xs) (if (cmp xs.head ys.head < 0) (_mergeSort_merge cmp xs.tail ys (Cons xs.head acc)) (_mergeSort_merge cmp xs ys.tail (Cons ys.head acc))))"
